...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    columnNames match {
      case s :: rest => StructType(
        StructField(s, StringType, nullable = false)
          :: rest.map(StructField(_, DoubleType, nullable = false))
      )
      case Nil => throw new IllegalArgumentException("Column name list is empty!")
    }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsColumns = columnNames.filter(colName => {
      colName.startsWith("t01") || colName.startsWith("t03") || colName.startsWith("t11") ||
        colName.startsWith("t1801") || colName.startsWith("t1803")
    }).map(new Column(_))

    val workColumns = columnNames.filter(colName => {
      colName.startsWith("t05") || colName.startsWith("t1805")
    }).map(new Column(_))

    val otherColumns = columnNames.filter(colName => {
      colName.startsWith("t02") || colName.startsWith("t04") || colName.startsWith("t06") ||
        ...
        (colName.startsWith("t18") &&
          !primaryNeedsColumns.contains(new Column(colName)) &&
          !workColumns.contains(new Column(colName)))
    }).map(new Column(_))
  ...
  def timeUsageSummary(primaryNeedsColumns: List[Column],
    ...
    val workingStatusProjection: Column = when($"telfs" >= 1.0 and $"telfs" < 3.0, "working")
